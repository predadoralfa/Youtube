...
module.exports = {
  async up(queryInterface) {
    await queryInterface.sequelize.transaction(async (transaction) => {
      ...
      if (!eraMinId) {
        throw new Error("Nao foi possivel localizar a Era 1 para seed da cesta Tier 4.");
      }

      const basketT4ItemDefId = await upsertByCode(queryInterface, transaction, "ga_item_def", "BASKET_T4", {
        name: "Basket Tier 4",
        category: "CONTAINER",
        stack_max: 1,
        unit_weight: 0.3,
      ...
      if (!basketT4ItemDefId) {
        throw new Error("Nao foi possivel seedar o item BASKET_T4.");
      }
      ...
      await upsertComponent(
        ...
        basketT4ItemDefId,
        "GRANTS_CONTAINER",
        JSON.stringify({ containerDefCode: "BASKET_T4" })
      );

      await upsertByCode(queryInterface, transaction, "ga_container_def", "BASKET_T4", {
        name: "Basket Pouch Tier 4",
        slot_count: 2,
        max_weight: 7.5,
  ...
  async down(queryInterface) {
    await queryInterface.sequelize.transaction(async (transaction) => {
      ...
      if (basketT4ItemDefId) {
        await queryInterface.bulkDelete("ga_item_def_component", { item_def_id: basketT4ItemDefId }, { transaction });
        await queryInterface.bulkDelete("ga_item_def", { id: basketT4ItemDefId }, { transaction });
      }

      await queryInterface.bulkDelete("ga_container_def", { code: "BASKET_T4" }, { transaction });
    });
  },
};
