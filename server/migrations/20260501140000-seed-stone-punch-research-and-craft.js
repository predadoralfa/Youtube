...
module.exports = {
  async up(queryInterface) {
    await queryInterface.sequelize.transaction(async (transaction) => {
      ...
      if (!eraMinId) {
        throw new Error("Nao foi possivel localizar a Era 1 para seedar Stone Punch.");
      }
      ...
      const stonePunchItemDefId = await findIdByCode(queryInterface, transaction, "ga_item_def", "STONE_PUNCH");
      ...
      if (!stonePunchItemDefId) throw new Error("Nao foi possivel localizar STONE_PUNCH.");
      if (!smallStoneItemDefId) throw new Error("Nao foi possivel localizar SMALL_STONE.");

      const researchDefId = await upsertResearchDef(queryInterface, transaction, "RESEARCH_STONE_PUNCH", {
        name: "Stone Punch",
        description: "Study the punching form of stone and unlock its craft.",
        item_def_id: stonePunchItemDefId,
        era_min_id: eraMinId,
        max_level: 1,
      ...
      if (!researchDefId) {
        throw new Error("Nao foi possivel seedar RESEARCH_STONE_PUNCH.");
      }

      await upsertResearchLevel(queryInterface, transaction, researchDefId, {
        level: 1,
        studyTimeMs: 3600000,
        title: "Stone Punch Crafting",
        description: "Unlock crafting the Stone Punch weapon.",
        grants: { unlock: ["recipe.craft:CRAFT_STONE_PUNCH"] },
        requirements: {
          itemCosts: [{ itemCode: "SMALL_STONE", qty: 40 }],
        },
      });

      const craftDefId = await upsertCraft(queryInterface, transaction, "CRAFT_STONE_PUNCH", {
        name: "Stone Punch",
        description: "Craft a stone melee weapon.",
        skill_def_id: skillDefId,
        required_skill_level: 1,
        required_research_def_id: researchDefId,
        required_research_level: 1,
        output_item_def_id: stonePunchItemDefId,
        output_qty: 1,
      ...
      if (!craftDefId) {
        throw new Error("Nao foi possivel seedar CRAFT_STONE_PUNCH.");
      }
  ...
  async down(queryInterface) {
    await queryInterface.sequelize.transaction(async (transaction) => {
      const researchDefId = await findIdByCode(queryInterface, transaction, "ga_research_def", "RESEARCH_STONE_PUNCH");
      if (researchDefId) {
        ...
        await queryInterface.bulkDelete(
          "ga_research_def",
          { id: researchDefId, code: "RESEARCH_STONE_PUNCH" },
          { transaction }
        );
      }

      const craftDefId = await findIdByCode(queryInterface, transaction, "ga_craft_def", "CRAFT_STONE_PUNCH");
      if (craftDefId) {
        await queryInterface.bulkDelete("ga_craft_recipe_item", { craft_def_id: craftDefId }, { transaction });
        await queryInterface.bulkDelete("ga_craft_def", { id: craftDefId, code: "CRAFT_STONE_PUNCH" }, { transaction });
      }
    });
  },
};
