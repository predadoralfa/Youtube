...
export function normalizeSpawnedActor(actor, store) {
  ...
  return {
    ...
    scale: {
      x: Number(actor?.scale?.x ?? actor?.scaleX ?? actor?.scale_x ?? 1),
      y: Number(actor?.scale?.y ?? actor?.scaleY ?? actor?.scale_y ?? 1),
      z: Number(actor?.scale?.z ?? actor?.scaleZ ?? actor?.scale_z ?? 1),
    },
  };
}
