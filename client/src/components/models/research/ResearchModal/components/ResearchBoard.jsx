...
function ResearchTreeNode({ node, inventoryIndex, onStartStudy, depth = 0 }) {
  ...
  return (
    <div className={`research-tree-node research-tree-node--depth-${depth}`}>
      ...
      {children.length > 0 ? (
        <div className={`research-tree-children research-tree-children--depth-${depth}`}>
          <div className="research-tree-link" aria-hidden="true">
            <span className="research-tree-link-line" />
            <span className="research-tree-link-dot" />
          </div>
          <div className="research-tree-children-stack">
            {children.map((child, childIndex) => (
              <ResearchTreeNode
                key={child.code ?? child.researchDefId ?? `${node.code}-child-${childIndex}`}
                node={child}
